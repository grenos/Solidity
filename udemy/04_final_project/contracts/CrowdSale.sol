...
contract Crowdsale is Context, ReentrancyGuard {
    ...
    /**
     * @param rate1 Number of token units a buyer gets per wei
     * @dev The rate is the conversion between wei and the smallest and indivisible
     * token unit. So, if you are using a rate of 1 with a ERC20Detailed token
     * with 3 decimals called TOK, 1 wei will give you 1 unit, or 0.001 TOK.
     * @param wallet1 Address where collected funds will be forwarded to
     * @param token1 Address of the token being sold
     */
    constructor (uint256 rate1, address payable wallet1, IERC20 token1) {
        require(rate1 > 0, "Crowdsale: rate is 0");
        require(wallet1 != address(0), "Crowdsale: wallet is the zero address");
        require(address(token1) != address(0), "Crowdsale: token is the zero address");
